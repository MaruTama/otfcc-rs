...
- **`otf_parse`** — sfnt/OTF binary parsing: `otfcc_read_sfnt` (table
  ...
  `RUST_MIGRATION.md`'s Phase 5 plan identified as having real, C-inherited
...
which stable rustc refuses). Nothing about the main crate's own `cargo
build`/`cargo test`/`cargo clippy` (run from the repo root) sees this
toolchain file or this workspace; they're isolated on purpose.
...
own limit of 10 (`libcff/cff_parser.rs`'s `MAX_SUBR_CALL_DEPTH`,
RUST_MIGRATION.md's Stage 7-4 file-I/O-adjacent CFF fix). Both files now parse
...
- ~~`table/cff.rs`'s `build_outline` allocated a fresh 0x10000-entry
  ...
  smaller); a companion test in `src/otf_reader.rs`
...
- ~~`table/cmap.rs`'s `read_format12` walks a group's `startCharCode..
  ...
  no-op for any well-formed group. See `RUST_MIGRATION.md`'s "Next steps" for
...
- ~~`tests/fuzz-corpus/known-issues/otf-parse-otl-contextual-amplification-
  ...
  individual rule. See `RUST_MIGRATION.md`'s "Next steps" for the full
...
- ~~`tests/fuzz-corpus/known-issues/otf-parse-otl-feature-ref-
  ...
  lookup/feature count were the actual dominant factors). See
  RUST_MIGRATION.md's "Next steps" for the full writeup, including how the
...
- ~~`tests/fuzz-corpus/known-issues/otf-dump-glyf-context-missing-maxp-
  ...
  *populated* when those tables are missing, not `font.head`/`font.maxp`
  ...
  the dump side. See `RUST_MIGRATION.md`'s "Next steps" for the full writeup.
...
- ~~`tests/fuzz-corpus/known-issues/otf-dump-ttinstr-pushw-orphaned-
  ...
  reproducing deterministically on the first try. See `RUST_MIGRATION.md`'s
