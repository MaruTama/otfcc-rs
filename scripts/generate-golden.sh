...
# crate. tests/golden.rs checks against it instead of rebuilding C from
# source on every run.
#
...
# changed in a `git diff` -- for that, use scripts/archive/compare-with-c.sh
...
# timestamp bytes even between two correct builds -- tests/dll_abi.rs
...
#      the C toolchain and running scripts/compare-with-c.sh, if the
...
#
# Invoke as: ./scripts/generate-golden.sh
set -euo pipefail
cd "$(dirname "$0")/../.."
. scripts/sha256-of.sh
...
# in tests/golden.rs for why it is not regenerated via fontTools here.
...
python3 scripts/make-test-unknown-lookup.py tests/payload/iosevka-r.ttf "${SCRATCH}/unknown-lookup-src.ttf"
...
synth_pair meta-test scripts/make-test-meta.py
synth_pair vdmx-test scripts/make-test-vdmx.py
synth_pair gsub-multi-dedup scripts/make-test-gsub-multi-dedup.py
synth_pair gpos-single-dedup scripts/make-test-gpos-single-dedup.py
synth_pair gpos-cursive-dedup scripts/make-test-gpos-cursive-dedup.py
synth_pair gdef-ligcaret-dedup scripts/make-test-gdef-ligcaret-dedup.py
synth_pair gsub-single-dedup scripts/make-test-gsub-single-dedup.py
synth_pair gsub-reverse-dedup scripts/make-test-gsub-reverse-dedup.py
synth_pair mark-consolidate-dedup scripts/make-test-mark-consolidate-dedup.py

echo "  dll-test.otf (kept as a real committed file, not a hash -- see header comment)"
SO_EXT="so"
[ "$(uname)" = "Darwin" ] && SO_EXT="dylib"
python3 scripts/test-dll.py "${BIN}/libotfcc_rust.${SO_EXT}" "${SCRATCH}/Molengo-Regular.json" "${GOLDEN}/dll-test.otf"
