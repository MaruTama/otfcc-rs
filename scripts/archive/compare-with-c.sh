...
# (target/release/), byte-for-byte, on the same canonical input JSON for
# each payload.
#
# MOVED HERE (scripts/archive/) when c/ (the C sources this script
# builds) was deleted from the tree -- see scripts/archive/README.md and
# the "Next steps" entry in RUST_MIGRATION.md for when and why. This script
# cannot run as-is any more: restore c/ from git history first (e.g.
# `git show <pre-deletion-commit>:c` or checking out a tag that predates the
# deletion) and copy it back to the repo root.
#
# NOT part of CI (see .github/workflows/rust.yml and
# scripts/compare-with-golden.sh): CI checks the Rust build against the
...
#
# Invoke as: ./scripts/archive/compare-with-c.sh (after restoring c/)
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
if command -v python3 >/dev/null 2>&1; then
	...
	python3 scripts/make-test-unknown-lookup.py tests/payload/iosevka-r.ttf "${UNKNOWN_LOOKUP}"
...
if command -v python3 >/dev/null 2>&1; then
	...
	python3 scripts/make-test-meta.py "${BUILD}/iosevka-r.json" "${META_JSON}"
...
if command -v python3 >/dev/null 2>&1; then
	...
	python3 scripts/make-test-vdmx.py "${BUILD}/iosevka-r.json" "${VDMX_JSON}"
...
if command -v python3 >/dev/null 2>&1; then
	...
	python3 scripts/make-test-base.py "${BUILD}/iosevka-r.json" "${BASE_JSON}"
