...
# scripts/archive/README.md. Restore c/ from git history first (e.g.
# `git show <pre-deletion-commit>:c`) for this to run.
#
# macOS:  ./scripts/archive/gen-compile-commands.sh
# Linux:  OS=linux ./scripts/archive/gen-compile-commands.sh
set -euo pipefail
cd "$(dirname "$0")/../../.."
...
node scripts/archive/filter-compdb.js /tmp/compdb.full.json c/compile_commands.json
