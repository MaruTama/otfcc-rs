...
#       --entrypoint bash otfcc-c2rust scripts/transpile.sh
set -euo pipefail
...
if [ ! -f c/compile_commands.json ]; then
	...
	echo "Generate it on the host first (see RUST_MIGRATION.md)." >&2
	exit 1
fi
...
# directly. See scripts/fix-transmute-abi.py for the implementation.
...
# built binary. See scripts/fix-float-narrowing.py for the target list.
