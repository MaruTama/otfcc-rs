...
# stderr output. tests/log_output.rs checks against these instead of
...
# golden.sh/tests/golden.rs already made for dump/build output. See
...
#
# Invoke as: ./scripts/generate-log-golden.sh
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# Same normalization as tests/log_output.rs's own normalize():
