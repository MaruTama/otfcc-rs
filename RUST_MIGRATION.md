...
This file documents the Rust port of otfcc, produced by transpiling the C
sources with [c2rust](https://c2rust.com/) and tracked in
[issue #2](https://github.com/MaruTama/otfcc/issues/2). The crate itself
lives at the repository root (`Cargo.toml`, `src/`, ...) — it used to be
nested one level down, under a `rust/` subdirectory, back when the C
implementation still lived side by side with it at the repo root; that
directory was hoisted up to the top once C was fully retired (see the
"Directory hoist" entry in "Next steps" below for when and why). This file
itself moved with it, from `rust/README.md` to `RUST_MIGRATION.md` at the
root, so as not to collide with the repo's own top-level `README.md`.

**The Rust source is committed source, not a build artifact.**
The plan was to eventually delete the C implementation and let this Rust
code be the real, standalone implementation — that already happened (see
below) — so unlike a typical generated-code directory, it's checked into
the repo and hand-edited directly. Only `target/` (the actual compiled
...
crate was transpiled from, and was deliberately kept side by side with the
Rust crate for exactly as long as it was needed as the byte-comparison
oracle (`compare-with-c.sh`). Once `tests/golden/` was frozen from a
confirmed-matching build — dump/build output via `checksums.sha256`
(`tests/golden.rs`), and stderr log output via `tests/golden/log/`
...
file) live side by side directly at the repo root — there is no separate
`transpiled/` subdirectory.
...
`fuzz/` (cargo-fuzz; needs a pinned nightly, its own toolchain file, see
`fuzz/README.md`) and `cargo +nightly-2026-08-17 miri test --lib --
...
idiomatization is allowed to change. `tests/golden.rs`/`dll_abi.rs`
...
`tests/abi.rs` keeps this honest: it fails if any of the four goes
...
- `archive/{gen-compile-commands.sh,filter-compdb.js}` — host-side: premake →
  ...
  `archive/README.md`.
- `tests/{abi,golden,log_output,cycles,lookup_alias,dll_abi}.rs` — the
...
- **`fuzz/README.md`'s last "known finding" (the `gvar-test.ttf`
  ...
  `otf_parse` corpus file. With this, every finding `fuzz/README.md`'s
...
- **`font/caryll_sfnt.rs`: fixed the last `fuzz`-`README.md`-documented
  ...
  This was scoped in `fuzz/README.md` as "exactly what Stage 7-1's
...
- **`table/cff.rs`/`libcff/charstring_il.rs`: fixed both of the two
  `json_build` bugs `fuzz/README.md`'s "Known findings" had documented
...
- **`table/cff.rs`: fixed a real leak in `callback_makefd`** (CID-keyed CFF
  ...
  - **Not caught locally beforehand**: LeakSanitizer isn't supported on
    ...
    `fuzz/corpus/json_build/` seed file -- `cid-fdselect-test.json`
...
- **`libcff/cff_parser.rs`: fixed the crash behind the `fuzz` CI job's
  ...
  - **`MAX_SUBR_CALL_DEPTH = 10`** (the Type 2 Charstring spec's own limit)
    ...
    stack-overflow that made `fuzz/README.md` exclude
  ...
  - **Found while verifying this fix**: `font/caryll_sfnt.rs:105`
    ...
    `fuzz/README.md`'s "Known findings" and
...
- **Stage 7-4: `font/caryll_sfnt.rs` moved off `libc::fopen`/`fread`/`fseek`
  ...
  - **Found while updating this function's one real caller**: the
    `otf_parse` fuzz target (`fuzz/`, a separate cargo workspace the
    ...
    `src/`. It used to wrap the fuzzer-provided byte slice with
...
- **Stage 7-2-b: `Options` owns its `Logger` inline, via `RefCell`, instead
  ...
  - **`fuzz/fuzz_targets/otf_parse.rs` needed the same treatment**
    as its two other `Options`-touching lines already got in Stage 7-2-a's
    tail. `fuzz/` is a separate cargo workspace member the normal
...
- **Stage 7-2-a (part 2): `*const Options` → `&Options`, the ~153
  ...
  - **What's left `*const Options` after all 4 batches, confirmed by
    ...
    Everything else in `src/` that ever took a used `Options`
...
- **Stage 7-2-a (part 1 of 2): removed 51 dead `_options: *const Options`
  ...
  (never read, never dereferenced) across `src/`; a fresh survey found
...
- **Stage 7-2-g: `libcff/subr.rs`'s intrusive doubly-linked-list CFF
  ...
  - `scripts/survey-unsafe.sh` deltas: raw pointer types 6261 → 6163
...
- **Stage 7-1 follow-up: `libcff/cff_parser.rs`'s `callsubr`/`callgsubr`
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 942 → 934 (-8),
...
- **Stage 7-1 follow-up: `libcff/cff_codecs.rs`'s Type2/DICT token
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 961 → 942 (-19),
...
- **Stage 7-1: `libcff/cff_parser.rs`, part 1 -- the CFF header and
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 973 → 961 (-12),
...
- **Stage 7-1: `libcff/cff_charset.rs`/`cff_fdselect.rs` -- second piece
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 982 → 973 (-9),
...
- **Stage 7-1: `libcff/cff_index.rs` -- the CFF INDEX reader, first piece
  ...
  - **No behavior change on any committed payload**: `KRName-Regular.otf`
    ...
    reads.
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1001 → 982 (-19),
...
- **Stage 7-1: `table/glyf/read.rs`, part 2 -- the gvar tuple-variation
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1029 → 1001 (-28),
...
- **Stage 7-1: `table/glyf/read.rs` -- the core glyf outline readers
  ...
  - **No behavior change on any committed payload**: every payload stayed
    ...
    and composite glyph.
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1080 → 1029 (-51),
...
- **Stage 7-1: `otl/subtables/chaining/read.rs` (1,246 lines) -- the
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1199 → 1080 (-119),
...
- **Stage 7-1: `otl/subtables/gpos_pair.rs` -- the last "regular" (non-
  ...
  - **No behavior change on any committed payload**: every payload
    ...
    each format.
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1240 → 1199, raw
...
- **Stage 7-1: `otl/subtables/{gsub_ligature,gpos_mark_to_ligature}.rs`
  ...
  - **No behavior change on any committed payload**: `KRName-Regular.otf`
    ...
    finding.
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1278 → 1240, raw
...
- **Stage 7-1: `otl/subtables/{gpos_single,gsub_reverse,gpos_mark_to_single}.rs`
  ...
  - **No behavior change on any committed payload**: the golden set's
    ...
    readers.
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1320 → 1278, raw
...
- **Stage 7-1: `otl/subtables/{gsub_single,gsub_multi,gpos_cursive}.rs`
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1352 → 1320, raw
...
- **Stage 7-1: `otl/read.rs` -- the top-level GSUB/GPOS/GDEF
  ...
  - `scripts/survey-unsafe.sh` deltas: `__fortable_*` 172 → 158,
...
- **Stage 7-1, start of the `otl`/ family: `otl/coverage.rs` and
  ...
  - `scripts/survey-unsafe.sh` deltas: `.offset(` 1452 → 1412, raw
...
- **Stage 7-1: `table/cmap.rs` -- the file the plan's original survey wrote
  ...
  - **No behavior change on any committed payload**: every payload with a
    ...
    a malformed `cmap`.
  - `scripts/survey-unsafe.sh` deltas: `__fortable_*` 186 → 172,
...
- **Stage 7-1, seventh batch and last of batch 2: `os_2`/`gasp`/`meta`/`vdmx`.**
  ...
  - `scripts/survey-unsafe.sh` deltas: `__fortable_*` 242 → 186,
...
- **Stage 7-1, sixth batch (start of batch 2): the six fixed-length
  ...
  - `scripts/survey-unsafe.sh` deltas: `__fortable_*` 326 → 242,
...
- **Stage 7-1, fifth batch and last of the original batch-1 list:
  ...
  - This closes out the plan's original Stage 7-1 batch-1 file list
    ...
    `otl`/`glyf` families.
  - `scripts/survey-unsafe.sh` deltas: `__fortable_*` 340 → 326,
...
- **Stage 7-1, fourth batch: `table/_tsi.rs::otfcc_read_tsi` — the dual-
  ...
  - `scripts/survey-unsafe.sh` deltas: `__fortable_*` 368 → 340,
...
- **Stage 7-1, third batch: `table/fpgm_prep.rs` + `table/tsi5.rs`.**
  ...
  - `scripts/survey-unsafe.sh` deltas: `__fortable_*` 394 → 368,
...
- **Stage 7-1, second batch: `table/{ltsh,hdmx,cvt}.rs` onto `FontReader`.**
  ...
  - `scripts/survey-unsafe.sh` deltas from this batch: `__fortable_*`
...
- **Phase 5, Stage 7-1 begins: `support/font_reader.rs` (`FontReader`), and
  ...
  - **New `scripts/survey-unsafe.sh`**, read-only, reports the
...
- **Phase 5, third PR: `cargo-fuzz` and `cargo miri test`, both wired into CI
  ...
  - **`fuzz/`** (new): two libFuzzer targets. `otf_parse` fuzzes the
  ...
  - **Found three real, unfixed bugs within the first ~20 seconds of
    running each target**, none fixed here (see `fuzz/README.md` for
  ...
  - **CI itself then caught three things local testing missed** — two are
    ...
    - **The `otf_parse` fuzz target's own harness had a real double-close
      ...
      in the fuzz harness `fuzz/fuzz_targets/otf_parse.rs` introduced
    ...
    - **With the double-close gone, `otf_parse` now runs far enough into
      ...
      steps in `fuzz/README.md`. Left leak detection **on** rather
...
- **Phase 5, second PR: clippy wired in with an allow-list ratchet, and CI
  ...
  - **CI's one `ubuntu-latest` job became a `strategy.matrix` over
    ...
    `compare-roundtrips.js`) on both. `RUST_MIGRATION.md` has said "verified on
...
- **Phase 5 kickoff: log output frozen as a golden fixture, then `c/` deleted.**
  ...
  - **`scripts/generate-log-golden.sh`** (new) captures the same six
...
- **`unsafe extern "C"` removal, Phase 1 of a new multi-phase cleanup: the
  ...
  the real public FFI boundary is exactly 4 functions in `src/ffi/dll.rs`
...
- **Stage 6-2's Logger vtable is retyped from `sds`/`SdsRaw` to `Vec<u8>`,
  ...
  - **New verification script:** `scripts/compare-log-output.sh`.
...
- **`ChainingRule.match_0` is `Vec<Coverage>` now, not `*mut *mut Coverage`
  ...
  - Verified with the standard full pipeline on both platforms (macOS arm64
    ...
    among `scripts/make-test-*-dedup.py`, but real coverage isn't
...
- **`Subtable` is a tagged `enum` now, not a `union`: done (B-1 of the
  ...
  - **Scope grew twice past the mechanical 92-site count the investigation
    ...
    - **`otf_writer/stat.rs`'s `stat_max_context_otl` was missed by the
      ...
      computation actually walk one), but `scripts/compare-with-c.sh`
      run against `target/release` (not the stale `bin/release-x64` copy
...
- **`LookupList` → `Vec<Box<Lookup>>`, third of the group — and it surfaced a
  ...
  - **Verified empirically, not just by reading, before deciding how to
    fix it**: `scripts/make-test-lookup-alias.py` (new) derives a
  ...
  - **Because C still crashes on this input, it cannot go through
    ...
    `scripts/test-lookup-alias.sh` instead: a Rust-only regression
...
- **uthash → real Rust containers: first instance done, real scope of the
  ...
  - **No committed payload has a `gsub_multiple` or `gsub_alternate` lookup
    ...
    duplicate-`from` path specifically. `scripts/
...
- **uthash → `BTreeMap`, second instance: `GposSingleHash`
  ...
  - **`NotoNastaliqUrdu-Regular.ttf`** (already in `tests/golden/`) **has
    ...
    `scripts/make-test-gpos-single-dedup.py` (new) forges one the same
...
- **uthash → `BTreeMap`, third instance: `GposCursiveHash`
  ...
  - `NotoNastaliqUrdu-Regular.ttf` has real `gpos_cursive` lookups, so the
    ordinary path was already covered; `scripts/
...
- **uthash → `BTreeMap`, fourth instance: `GdefLigCaretHash`
  ...
  - **`NotoNastaliqUrdu-Regular.ttf`** has real (14-entry) `ligCarets`, so
    ...
    comparison. `scripts/make-test-gdef-ligcaret-dedup.py` (new, same
...
- **uthash → `BTreeMap`, fifth and sixth instances: `GsubSingleMapHash`,
  ...
  - **`consolidate_gsub_reverse` turned up a genuine, if narrow,
    ...
    *before* reading those aliases back out to build the final handles.
    ...
    empirically: `scripts/make-test-gsub-reverse-dedup.py` places its
  ...
  - Neither function has an existing payload exercising its *ordinary*
    ...
    duplicate `from`. `scripts/make-test-gsub-single-dedup.py` (new,
    the established duplicate-JSON-key technique, since `gsub_single`'s
    subtable is a JSON object) and `scripts/
...
- **uthash → `HashMap`, eighth instance: `JsonObjEntry`
  ...
  - **Real functional coverage, not synthetic**: `json_ident`'s only
    ...
    of duplicating it — exactly what `scripts/test.sh`'s
...
- **uthash → `IndexMap`, ninth instance: `FvarMaster`
  ...
  *vtable slot field name* too.
  ...
  - This is the crate's first dependency on `indexmap` (added to
    `Cargo.toml`, pulling in `hashbrown`/`equivalent` transitively).
  - Verified with the standard full pipeline on both macOS and Linux.
...
- **uthash → `IndexMap`, twelfth instance: `CffSidEntry`
  ...
  - **Real functional coverage across both branches, not synthetic**:
    ...
    (the "fj" — from-JSON — payload already in `scripts/
...
- **uthash → `BTreeMap`, fifteenth instance: `BaseHash`/`MarkHash`/`LigHash`
  (`src/consolidate/otl/mark.rs`)**. Three separate uthash tables in
  ...
  - **No coverage gap needed real chasing this time, but no free ride
    ...
    under test. Closed with `scripts/make-test-mark-consolidate-dedup.py`
...
- **uthash → `IndexSet`/`IndexMap`, sixteenth instance: `CoverageEntry`
  (`src/table/otl/coverage.rs`'s `read_coverage`, plus a second,
  cross-file copy of the *same* struct in `src/table/otl/classdef.rs`
...
- **uthash → `BTreeMap`, seventeenth instance: `ClassifierHash`
  (`src/table/otl/subtables/chaining/classifier.rs`)**, the OTL
...
- **uthash → `Vec`, eighteenth instance: `LookupHash`
  (`src/table/otl/parse.rs`)**. `parse.rs` (6,692 lines) holds three
  ...
  - **Real, direct coverage for both mechanisms this instance touches**:
    `scripts/make-test-lookup-alias.py` (already part of every
...
- **uthash → `Vec`, nineteenth instance: `FeatureHash`
  (`src/table/otl/parse.rs`)**, the second of the three name-keyed
...
- **uthash → `BTreeMap`, twentieth instance: `LanguageHash`
  (`src/table/otl/parse.rs`)**, the third and last of this file's
  name-keyed hashes.
...
- **uthash → `BTreeMap`, twenty-first instance (first half): `CmapTable.unicodes`
  (`src/table/cmap.rs`)**, converting the `CmapEntry` hash to
...
- **uthash → `BTreeMap`, twenty-second instance (second half):
  `CmapTable.uvs` (`src/table/cmap.rs`)**, converting the
...
- **uthash → `HashMap`, twenty-third instance: `CffSubrGraph.diagram_index`
  (`src/libcff/subr.rs`)**, converting the `CffSubrDiagramIndex` hash
...
- **uthash → `BTreeMap`/`HashMap`, twenty-fourth instance:
  `GlyphOrder.by_gid`/`by_name` (`src/support/glyph_order.rs`)**, the
...
- **Then safe Rust, type by type. Stage 6-1 (`CVecRaw<T>` → `Vec<T>`,
  ...
  - **`CaretValueList`/`LigCaretTable` → `Vec<CaretValue>`/
    ...
    - `table_gdef_create` was still using `malloc`, and `init_gdef` assigns
      ...
      `GaspTable` hazard (RUST_MIGRATION.md), caught and fixed proactively
...
- **CI decoupled from C: `tests/golden/` now carries `compare-with-c.sh`'s
  ...
  - **What moved**: for every payload `compare-with-c.sh` already covered
    ...
    `tests/golden/checksums.sha256`. `scripts/compare-with-golden.sh`
  ...
  - **`compare-with-c.sh` itself is not deleted** — it still builds C and
    ...
    running the new `scripts/generate-golden.sh` to refresh
...
- **Stage 6-4 "Box化", batched: `base`/`gdef` (2 `Font` fields) in one PR**,
  ...
  - **Verification gap found and closed**: no existing payload has a
    ...
    `scripts/make-test-base.py` (two scripts, one with a default
...
- **Directory hoist (2026-09-16): the crate moved from `rust/` to the
  repository root.** With the C implementation fully retired (see this
  file's opening paragraph), there was no longer any reason for the Rust
  port to live nested one level below the repo root — that nesting only
  ever existed to let the C and Rust trees coexist side by side during the
  migration. `Cargo.toml`/`Cargo.lock`/`build.rs`/`rust-toolchain.toml`/
  `src/`/`scripts/`/`fuzz/` moved to the repo root via `git mv`; this file
  itself moved from `rust/README.md` to `RUST_MIGRATION.md` at the root
  (to avoid colliding with the repo's own pre-existing `README.md`, which
  now links here); `rust/tests/*.rs` and `rust/tests/support/` merged into
  the existing root `tests/` directory (which previously held only data
  fixtures — `golden/`, `payload/`, `fuzz-corpus/` — with zero filename
  collisions against the incoming test files). The now-empty `rust/`
  directory (including its gitignored, multi-gigabyte `target/` build
  cache) was deleted outright.
  - **The one functional fix this required** (not just doc/path
    updates): `tests/support/mod.rs`'s `repo_root()` used to call
    `.parent()` on `CARGO_MANIFEST_DIR`, because the crate root
    (`rust/`) used to sit one level *below* the true repo root. Now that
    the crate root and the repo root are the same directory,
    `CARGO_MANIFEST_DIR` already *is* the repo root — the `.parent()`
    call was removed. `tests/abi.rs`'s analogous `manifest_path()`
    helper needed no change: it only ever joined a path *relative to*
    `CARGO_MANIFEST_DIR`, so the shift was transparent to it.
  - **A second, more subtle fix**: a handful of tests (`otf_reader.rs`'s
    `regression_tests` module, `support/parsed_json.rs`'s
    `every_committed_payload_json_parses`) read fixtures via paths
    relative to the test binary's working directory (which `cargo test`
    sets to `CARGO_MANIFEST_DIR`), written as `"../tests/fuzz-corpus/
    ..."` / `"../tests/payload"` / `"../build"` — correct when
    `CARGO_MANIFEST_DIR` was `rust/`, one level below the fixtures, but
    now one level too high. Caught by actually running the full test
    suite (not just `build`/`clippy`) after the hoist — 6 tests failed
    with `NotFound` I/O errors — and fixed by dropping the stale `../`
    from all six occurrences.
  - `.github/workflows/rust.yml` lost every `cd rust`/`cd rust/fuzz`
    step and had its trigger `paths:` updated from `["tests/**",
    "rust/**", ...]` to the individual top-level dirs/files that now
    exist at the root (`src/**`, `fuzz/**`, `scripts/**`, `Cargo.toml`,
    etc.). `.gitignore`'s `rust/target/` and
    `rust/.compile_commands.native.json` entries became `target/` and
    `.compile_commands.native.json`.
  - Every other `rust/`-prefixed path reference crate-wide (doc
    comments, this file's own prose, `fuzz/README.md`, script
    comments) was updated to its new root-relative spelling, found via
    `git grep -n "rust/"` and fixed with a bulk `git grep -l ... | xargs
    sed` pass (a plain bash `for` loop around repeated `sed -i` calls
    silently failed to apply most edits in this environment — diagnosed
    by comparing before/after `git grep -c` counts — so a single
    null-delimited `xargs` invocation was used instead) plus targeted
    `Edit` calls for multi-line-wrapped comments. Left deliberately
    unchanged, per this file's append-only convention: every historical
    PR-log entry above this one that mentions `rust/`-prefixed paths (an
    accurate record of what was true when it was written), and
    `scripts/archive/*`'s references (the archived transpile/compare
    tooling is out of maintenance scope).
  - Verified with the standard full pipeline post-hoist, run from the
    new root layout: `cargo build`/`clippy` clean, all 395 lib tests and
    the full integration suite (`abi`/`cycles`/`dll_abi`/`golden`/
    `log_output`/`lookup_alias`) green, `cargo miri test` 359 passed/0
