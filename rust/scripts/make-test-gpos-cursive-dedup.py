#!/usr/bin/env python3
"""Forge a GPOS "cursive attachment" lookup whose single subtable has two
rules for the *same* target glyph -- the case `consolidate_gpos_cursive`
(rust/src/consolidate/otl/gpos_cursive.rs) dedupes: first occurrence wins, a
"[Consolidate] Double-mapping a glyph in a cursive positioning" warning is
logged, and the second rule is dropped (not merged).

`NotoNastaliqUrdu-Regular.ttf` (in tests/golden/) already has real
gpos_cursive lookups, so the ordinary path is covered -- but none of the
committed payloads has a *duplicate* target glyph within one subtable, so
this dedup branch has never actually run under test. Same technique as
make-test-gsub-multi-dedup.py / make-test-gpos-single-dedup.py: a
...
collapse, but otfcc's own vendored parser does not -- see
make-test-gsub-multi-dedup.py's docstring for the full explanation.

Usage: make-test-gpos-cursive-dedup.py <in.json> <out.json>
"""
import json
import sys
...
def main():
    ...
    lookup_name = "lookup_test_gpos_cursive_dedup"
    ...
    # first (enter x=111) is expected to survive consolidation, the second
    # (enter x=222) must be dropped, not merged, not averaged.
    raw_subtable_array = (
        "["
        "{"
        f'"{target_glyph}":{{"enter":{{"x":111,"y":0}},"exit":null}},'
        f'"{target_glyph}":{{"enter":{{"x":222,"y":0}},"exit":null}}'
        "}"
        "]"
    )
    ...
    with open(dst, "w") as f:
        f.write(text)
    print(
        f"  wrote {dst} (gpos_cursive lookup, one subtable, duplicate "
        f"target glyph {target_glyph!r}: first rule's enter.x 111 must win "
        f"over the second rule's enter.x 222)"
    )
