//! Rust-native replacement for `rust/scripts/test-lookup-alias.sh` (see
//! that script's own header, and `rust/scripts/make-test-lookup-alias.py`,
...
fn gsub_lookup_alias_does_not_duplicate_or_destabilize() {
    ...
    run_ok(
        ...
        Path::new("python3"),
        &[repo_root().join("rust/scripts/make-test-lookup-alias.py").as_os_str(), src_json.as_os_str(), alias_json.as_os_str()],
    );
