...
//! `rust/scripts/test-dll.py`, `rust/scripts/dll-arch-check.sh`, and the
//! `otfccdll` sections of `rust/scripts/compare-with-golden.sh` and
//! `rust/scripts/run-cycles.sh` that invoke them. Loads the four public
//! ABI functions (`rust/src/ffi/dll.rs`) from the just-built cdylib via
