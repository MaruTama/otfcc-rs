...
pub unsafe fn otfcc_build_meta(
    ...
    let mut root: *mut BkBlock = unsafe { bk_new_block(&[bk_int(BkCellType::B32, (meta.version) as u32), bk_int(BkCellType::B32, (meta.flags) as u32), bk_int(BkCellType::B32, 0 as u32), bk_int(BkCellType::B32, (entries.len() as u32) as u32)]) };
    let mut __caryll_index: usize = 0 as usize;
    let mut keep: usize = 1 as usize;
    while keep != 0 && __caryll_index < entries.len() {
        let e: &MetaEntry = &entries[__caryll_index];
        while keep != 0 {
            unsafe { bk_push(root, &[bk_int(BkCellType::B32, (e.tag) as u32), bk_ptr(BkCellType::P32, bk_new_block_from_string_len(
                    e.data.len(),
