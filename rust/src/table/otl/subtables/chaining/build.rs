...
pub unsafe fn otfcc_chaining_lookup_is_contextual_lookup(mut lookup: *const Lookup) -> bool {
...
pub unsafe fn otfcc_build_chaining_classes(mut _subtable: *const ChainingSubtable) -> *mut Buffer {
    ...
    while (j_0 as usize) < (*ruleset).rules.len() {
        ...
        let mut start_class: TableId = (&(*rule_j0).match_0)[ib as usize][0].index as TableId;
    ...
    while j_1 as ::core::ffi::c_int <= (*ic).maxclass as ::core::ffi::c_int {
        if rcpg[j_1 as usize] != 0 {
            let mut cset: *mut BkBlock = bk_new_block(&[bk_int(
                BkCellType::B16,
