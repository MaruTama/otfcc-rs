...
use crate::table::otl::{ChainingRule, Lookup, Subtable, SubtablePtr, ChainingType, OTL_TYPE_GPOS_CHAINING, OTL_TYPE_GSUB_CHAINING, ChainingSubtable};
...
pub unsafe extern "C" fn otfcc_chaining_lookup_is_contextual_lookup(
    ...
    while (j as usize) < (*lookup).subtables.len() {
        ...
        let mut subtable: *const ChainingSubtable = &raw const (*subtable_ptr).chaining;
...
pub unsafe extern "C" fn otfcc_build_chaining_coverage(
    ...
    let mut n_subst: TableId = (*rule).apply_count;
    ...
    bk_push(root, &[bk_int(BkCellType::B16, ((*rule).apply_count as ::core::ffi::c_int) as u32)]);
...
pub unsafe extern "C" fn otfcc_build_chaining_classes(
    ...
    (*coverage).num_glyphs = (*(*subtable).c2rust_unnamed.c2rust_unnamed.ic).num_glyphs;
        ...
        if *rcpg.offset(j_1 as isize) != 0 {
                ...
                < (*subtable).c2rust_unnamed.c2rust_unnamed.rules_count as ::core::ffi::c_int
            {
                ...
                let mut start_class_0: GlyphClass =
                    (*(**(*rule).match_0.offset((*rule).input_begins as isize))
                        .glyphs
                        .offset(0 as ::core::ffi::c_int as isize))
                    .index as GlyphClass;
